...
function deleteArticle(url, request) {
  ...
  if (savedArticle) {
    database.articles[id] = null; //set article at id equal to null
    savedArticle.commentIds.forEach(commentId => { //for each comment ID
      const comment = database.comments[commentId];
      database.comments[commentId] = null; //set comment to null
...
function getComments(url, request) {
  ...
  response.body = {
    comments: Object.keys(database.comments)
        .map(commentId => database.coments[commentId])
...
function getComment(url, request) {
  ...
  if (comment) {
    comments.article = comments.articleId.map(
      articleId => database.articles[articleId]);
